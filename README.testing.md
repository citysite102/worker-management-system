...
1. **前端元件測試仍然很少** —— 目前只有 `exportToCsv` 的單元測試。
   12 個頁面與所有 component 都還沒有元件層級的測試，E2E 仍是主要的前端防線。
   下一步建議從 `CaseModal` 的 `missingFields` 計算、各 Modal 的表單驗證開始。
2. **`data-testid` 只補了 Cases 頁與 CaseModal** —— Workers / Customers /
   Dashboard 等頁面還沒補，那些頁面的 E2E 之後仍得靠文字選取。
