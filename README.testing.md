...
**改 schema 之後要重啟 dev server。** `drizzle-kit push` 是 DDL，MySQL 會讓
連線池裡已快取的 prepared statement 失效，但長時間執行的 server 還握著它們，
下一次查詢就會噴 `Failed query: ...`。實際發生過：加 FK 約束時 dev server
還開著，儀表板的 KPI 快照查詢就整個掛掉，直到 server 重啟才恢復。
