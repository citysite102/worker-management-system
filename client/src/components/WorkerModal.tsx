...
function daysFromToday(dateStr: string | undefined): number | null {
  ...
  // 共用 shared/expiry（台北時區），與 Workers/NotificationBell 一致；非法日期回 null。
  const d = daysUntil(dateStr, todayInTaipei());
...
function DaysLeftBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-xs font-medium mt-1 ${color}`}
    >
  ...
  const validateFileType = useCallback(
    (file: File): boolean => {
      ...
      if (!isAllowed) {
        if (isImageOnly) {
          toast.error(
            `此欄位僅支援圖片格式（JPG、PNG、WEBP 等），請勿上傳 PDF`
          );
        } else {
...
export function WorkerModal({
  ...
  const [permitDuplicateError, setPermitDuplicateError] = useState<
    string | null
  >(null);
  const [passportDuplicateError, setPassportDuplicateError] = useState<
    string | null
  >(null);
  ...
  const permitNoStatus = !watchedPermitNo
    ? "empty"
    : validateResidentPermit(watchedPermitNo)
      ? "valid"
      : "invalid";
  const passportNoStatus = !watchedPassportNo
    ? "empty"
    : validatePassport(watchedPassportNo)
      ? "valid"
      : "invalid";
  ...
  useEffect(() => {
    ...
    if (isEdit && existingWorker) {
      ...
      reset({
        ...
        managerId: String(w.managerId),
        residentPermitNo: w.residentPermitNo ?? "",
        residentPermitExpiry: w.residentPermitExpiry ?? "",
        passportNo: w.passportNo ?? "",
        passportExpiry: w.passportExpiry ?? "",
        entryDate: w.entryDate ?? "",
        phone: w.phone ?? "",
        email: w.email ?? "",
        lastMedicalExamDate: w.lastMedicalExamDate ?? "",
        nextMedicalExamType: w.nextMedicalExamType ?? "",
        photoKey: w.photoKey ?? "",
        ktpKey: w.ktpKey ?? "",
        residentPermitFrontKey: w.residentPermitFrontKey ?? "",
        residentPermitBackKey: w.residentPermitBackKey ?? "",
        passportKey: w.passportKey ?? "",
        passportEntryKey: w.passportEntryKey ?? "",
        medicalReportKey: w.medicalReportKey ?? "",
  ...
  const onSubmit = async (data: WorkerFormData) => {
    ...
    if (!data.nameCn.trim() && !data.nameEn.trim()) {
      toast.error("請填寫中文或英文姓名");
      return;
    }
      ...
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        >
          ...
          <div className="space-y-4 pb-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label>英文姓名</Label>
                <Input
                  data-testid="worker-modal-name-en"
                  placeholder="INTAN SUSELA"
                  {...register("nameEn")}
                  {...enterProps}
                  autoComplete="off"
                />
              ...
              <div className="space-y-2">
                <Label>中文姓名</Label>
                <Input
                  data-testid="worker-modal-name-cn"
                  placeholder="白茵瑤"
                  {...register("nameCn")}
                  {...enterProps}
                  autoComplete="off"
              ...
              <div className="space-y-2">
                <Label>
                  目前狀態 <span className="text-destructive">*</span>
                </Label>
                <Select
                  value={watch("lifecycleStatus")}
                  onValueChange={v => setValue("lifecycleStatus", v)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={v => setValue("documentStatus", v)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger data-testid="worker-modal-manager">
                ...
                <div className="relative">
                  <Input
                    ...
                    onChange={e => {
                      ...
                      setValue("residentPermitNo", upper, {
          ...
          <div className="space-y-4 py-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={watch("nextMedicalExamType")}
                  onValueChange={v => setValue("nextMedicalExamType", v)}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="請選擇" />
                  </SelectTrigger>
                  <SelectContent>
                    {MEDICAL_EXAM_TYPE_OPTIONS.map(o => (
                      <SelectItem key={o.value} value={o.value}>
                        {o.label}
                      </SelectItem>
                    ))}
              ...
              {nextMedicalExamDate && (
                <div className="sm:col-span-2 flex flex-wrap gap-5 p-3 bg-muted/30 rounded-md text-sm border border-border/50">
                  ...
                  <div>
                    ...
                    <p
                      className={`font-medium mt-0.5 ${nextMedicalExamDays !== null && nextMedicalExamDays < 0 ? "text-red-500" : nextMedicalExamDays !== null && nextMedicalExamDays <= 30 ? "text-amber-500" : ""}`}
                    >
                      {nextMedicalExamDays !== null
                        ? nextMedicalExamDays < 0
                          ? `已過期 ${Math.abs(nextMedicalExamDays)} 天`
                          : `${nextMedicalExamDays} 天`
                        : "—"}
                    </p>
                  </div>
                </div>
          ...
          <div className="space-y-4 py-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FileUploadField
                ...
                currentKey={photoKey}
                onUploaded={k => setValue("photoKey", k)}
                accept="image/*"
              />
              <FileUploadField
                label="母國身分證（KTP）"
                ...
                workerId={savedWorkerId}
                currentKey={ktpKey}
                onUploaded={k => setValue("ktpKey", k)}
                accept="image/*"
              />
              <FileUploadField
                label="居留證正面"
                fieldName="residentPermitFrontKey"
                workerId={savedWorkerId}
                currentKey={residentPermitFrontKey}
                onUploaded={k => setValue("residentPermitFrontKey", k)}
                accept="image/*"
              />
              <FileUploadField
                label="居留證背面"
                fieldName="residentPermitBackKey"
                workerId={savedWorkerId}
                currentKey={residentPermitBackKey}
                onUploaded={k => setValue("residentPermitBackKey", k)}
                accept="image/*"
              />
              <FileUploadField
                label="護照"
                fieldName="passportKey"
                workerId={savedWorkerId}
                currentKey={passportKey}
                onUploaded={k => setValue("passportKey", k)}
                accept="image/*"
              />
              <FileUploadField
                label="護照入境頁"
                fieldName="passportEntryKey"
                workerId={savedWorkerId}
                currentKey={passportEntryKey}
                onUploaded={k => setValue("passportEntryKey", k)}
                accept="image/*"
              />
              <FileUploadField
                label="體檢報告"
                fieldName="medicalReportKey"
                workerId={savedWorkerId}
                currentKey={medicalReportKey}
                onUploaded={k => setValue("medicalReportKey", k)}
                accept="image/*,application/pdf"
              />
            </div>
          </div>
                ...
                <div className="flex gap-2">
                  ...
                  {watch("externalLink") && (
                    <Button
                      ...
                      className="shrink-0"
                      onClick={() =>
                        window.open(watch("externalLink"), "_blank")
                      }
                    >
          ...
          <DialogFooter className="pt-2 border-t">
            ...
            <Button
              data-testid="worker-modal-cancel"
              type="button"
              variant="outline"
              onClick={onClose}
            >
            ...
            {missingFields.length > 0 ? (
              <Tooltip>
                <TooltipTrigger asChild>
                  <span
                    data-testid="worker-modal-submit-wrap"
                    tabIndex={0}
                    className="inline-flex"
                  >
                    <Button
                      data-testid="worker-modal-submit"
                      type="submit"
                      disabled
                ...
                <TooltipContent
                  data-testid="worker-modal-missing-fields"
                  side="top"
