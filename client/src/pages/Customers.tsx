...
export default function Customers() {
  ...
  const filtered = useMemo(() => {
    const list = customers.filter(c => {
      ...
      const matchManager =
        managerFilter === "all" || String(c.managerId) === managerFilter;
      const matchContract =
        contractFilter === "all" || c.contractStatus === contractFilter;
      const matchPricing =
        pricingFilter === "all" || c.pricingTier === pricingFilter;
      const matchEmployerType =
        employerTypeFilter === "all" ||
        (ca.employerType ?? "company") === employerTypeFilter;
      return (
        matchSearch &&
        matchManager &&
        matchContract &&
  ...
  const stats = useMemo(
    () => ({
      ...
      individual: customers.filter(
        c => (c as any).employerType === "individual"
      ).length,
      company: customers.filter(
        c => (c as any).employerType === "company" || !(c as any).employerType
      ).length,
      inService: customers.filter(c => c.contractStatus === "in_service")
        .length,
      negotiating: customers.filter(c => c.contractStatus === "negotiating")
        .length,
      pendingRenewal: customers.filter(
        c => c.contractStatus === "pending_renewal"
      ).length,
    }),
    [customers]
  );
  ...
  const handleStatClick = (
    type: "in_service" | "negotiating" | "pending_renewal"
  ) => {
    clearAllFilters();
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
        {/* 總數卡（不可點擊篩選） */}
        <div
          data-testid="customers-stat-total"
        ...
        >
          ...
          <p
            data-testid="customers-stat-value"
        ...
        {[
          ...
          return (
            <button
              ...
              type="button"
              onClick={() =>
                active ? clearAllFilters() : handleTypeClick(card.type)
              }
        ...
        {[
          ...
          // 洽談中 → 警示，唯一強調色
          {
            ...
            icon: MessageSquare,
            warn: true,
            filterVal: "negotiating" as const,
          },
          // 待續約 → 警示，唯一強調色
          {
            label: "待續約",
            ...
            icon: RefreshCw,
            warn: true,
            filterVal: "pending_renewal" as const,
          },
        ].map(card => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() =>
                active ? clearAllFilters() : handleStatClick(card.filterVal)
              }
              className={`bg-card border rounded-lg p-4 text-left transition-all hover:shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
                active
                  ? card.warn
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <card.icon
                  className={`w-4 h-4 ${card.warn ? "text-amber-500" : "text-muted-foreground"}`}
                />
              </div>
              <p
      ...
      <div className="space-y-2">
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <Select value={contractFilter} onValueChange={setContractFilter}>
            <SelectTrigger
              data-testid="customers-filter-contract"
              className="w-full sm:w-36"
            >
          ...
          <Select
            ...
            onValueChange={setEmployerTypeFilter}
          >
            <SelectTrigger
              data-testid="customers-filter-employer-type"
        ...
        {hasActiveFilter && (
          <div
            data-testid="customers-filter-summary"
                ...
                filtered.map(c => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 hidden xl:table-cell">
                      {(c as any).caseStatus ? (
                        <StatusBadge status={(c as any).caseStatus} />
                      ) : (
